...
bash 'generate-and-sign-key' do
  ...
  code <<-EOH
    ...
    openssl req -subj "/CN=system:node:#{node['fqdn']}/O=system:nodes" -passin pass:#{node['kube-hops']['hopsworks_cert_pwd']} -passout pass:#{node['kube-hops']['hopsworks_cert_pwd']} -key hopsworks.key.pem -new -sha256 -out hopsworks.csr.pem
  EOH
